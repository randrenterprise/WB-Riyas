...
// ============= CONFIGURATION =============
// FIREBASE_URL will be loaded from GitHub Secrets
const FIREBASE_URL = process.env.FIREBASE_URL;
const ADMIN_NUMBER = "+94 75 483 483 4"; // REPLACE WITH YOUR WHATSAPP NUMBER
// ==========================================

const userStates = {};
const userTimeouts = {};

// Helper: Fetch timeout settings
...
async function createServiceRequest(userId, service, customerDetails, issue) {
    ...
    await fetch(`${FIREBASE_URL}/service_requests.json`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function startBot() {
    if (!FIREBASE_URL) {
        ...
        console.log("📍 Add FIREBASE_URL to: Settings → Secrets and variables → Actions");
        process.exit(1);
    }

    console.log("🚀 Starting CCTV & Electrical Services WhatsApp Bot...");
    ...
    const sock = makeWASocket({
        ...
        browser: ["WhatsApp", "ServiceBot", "1.0"]
    });
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            console.log('\n✅ BOT IS ONLINE!');
            console.log(`📱 Ready to handle service requests!`);
            
            if (ADMIN_NUMBER && ADMIN_NUMBER !== "1234567890") {
                try {
                    await sock.sendMessage(ADMIN_NUMBER + '@s.whatsapp.net', { 
                        text: `🔧 *Service Bot Online!*\n\n✅ Ready to handle CCTV, Electrical & Maintenance requests.\n📅 Time: ${new Date().toLocaleString()}` 
                    });
